...
# ──────────────────────────────────────────────────────────────
# NEW: advance_thumbnail.py style — blurred BG + circle crop
# ──────────────────────────────────────────────────────────────

def crop_center_circle(img, output_size, border, crop_scale=0.5):
    """Crop image to a white-bordered circle (advance_thumbnail style)."""
    half_w = img.size[0] / 2
    half_h = img.size[1] / 2
    larger_size = int(output_size * crop_scale)

    # 1. Crop centre square
    img = img.crop((
    ...
    # 2. Resize to inner size (excluding border)
    inner = output_size - 2 * border
    ...
    ImageDraw.Draw(mask_inner).ellipse((0, 0, inner, inner), fill=255)
    final_img.paste(img, (border, border), mask_inner)

    # 5. Outer circle mask (for transparency)
    ...
    return Image.composite(
        final_img,
        Image.new("RGBA", final_img.size, (0, 0, 0, 0)),
        mask_outer,
    )


def build_advanced_background(base_img, canvas_w, canvas_h):
    """
    Returns an RGBA canvas that is:
      • the YouTube thumbnail stretched & blurred (advance_thumbnail style)
      • darkened with Brightness(0.6)
    This replaces the solid-black + gradient background ONLY for the
    base canvas layer.  All other layers (gradient overlay, particles,
    text …) are still composited on top exactly as before.
    """
    bg = base_img.resize((canvas_w, canvas_h)).convert("RGBA")
...
# ──────────────────────────────────────────────────────────────
# Unchanged helpers from original thumbnails.py
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# Main function — only the canvas initialisation & art paste
# section changed; everything else is identical to original
# ──────────────────────────────────────────────────────────────
...
async def gen_thumb(videoid: str):
    ...
    try:
        # ── CHANGED: blurred-BG canvas (advance_thumbnail style) ──
        canvas = build_advanced_background(base_img, CANVAS_W, CANVAS_H)
        # ─────────────────────────────────────────────────────────
        ...
        # ── CHANGED: white-border circle crop (advance_thumbnail style) ──
        BORDER = 18  # white border thickness (matches advance_thumbnail THUMB_BORDER=20)
        circle_art = crop_center_circle(base_img, art_size, BORDER)
        # ─────────────────────────────────────────────────────────────────

        if random.choice([True, False]):
            add_glow_ring(canvas, art_x, art_y, art_size, accent_color, random.randint(8, 15))

        # ── CHANGED: paste the circle-cropped art instead of shape-masked art ──
        canvas.paste(circle_art, (art_x, art_y), circle_art)
        # ───────────────────────────────────────────────────────────────────────
